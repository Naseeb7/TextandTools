...
export default function TextForm(props) {
  ...
  //     let newword=new RegExp(searchvalue,"g"); // search for all instances
	// 	let newText = text.replace(newword, `<mark>${searchvalue}</mark>`);
  ...
  //  };
  //  if(text!==""){
  //    if(findAll(text.split(" "),searchvalue)===0){
  //      document.getElementById("occurances").innerHTML="Not in the text!"
  //     }
  //     else{
  //       document.getElementById("occurances").innerHTML=`Number of occurances : ${findAll(text.split(" "),searchvalue)}`
...
const handleCopy=()=>{
  ...
  <textarea  id="textarea" value={text} onChange={handleonChange} style={props.mode} rows="8" placeholder='Enter text here'></textarea>
